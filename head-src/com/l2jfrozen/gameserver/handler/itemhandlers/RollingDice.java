...
public class RollingDice implements IItemHandler
{
	...
	public void useItem(final L2PlayableInstance playable, final L2ItemInstance item)
	{
		...
		if (itemId == 4625 || itemId == 4626 || itemId == 4627 || itemId == 4628)
		{
			...
			sm.addNumber(number);
			if (activeChar.isInDiceEvent()){
				if (activeChar.getDiceEvent().results.get(activeChar) == 0){
					activeChar.getDiceEvent().results.put(activeChar, number);
				} else {
					activeChar.sendMessage("You already threw the dice ur score is : " + activeChar.getDiceEvent().results.get(activeChar));
				}
			}
			activeChar.sendPacket(sm);
