...
public class Potions implements IItemHandler
{
	...
	public synchronized void useItem(final L2PlayableInstance playable, final L2ItemInstance item)
	{
		...
		if (playable instanceof L2PcInstance)
		{
			...
			if (activeChar._inEventBR && BattleRoyale.is_started() && !Config.BR_ALLOW_POTIONS)
			{
				activeChar.sendPacket(ActionFailed.STATIC_PACKET);
				return;
			}
		...
		if (player instanceof L2PcInstance)
		{
			...
			else
			{
				...
				if (skill != null)
				{
					...
					/*
					if(skill.getId()==2287 && level==6){
						final L2ItemInstance item = activeChar.getInventory().getItemByItemId(8627);
						if(item!=null) {
							activeChar.destroyItem("Consume", item.getObjectId(), 1, null, false);
						}
					}else if(skill.getId()==2288 && level==6){
						final L2ItemInstance item = activeChar.getInventory().getItemByItemId(8633);
						if(item!=null) {
							activeChar.destroyItem("Consume", item.getObjectId(), 1, null, false);
						}
					}else if(skill.getId()==2289 && level==6){
						final L2ItemInstance item = activeChar.getInventory().getItemByItemId(8639);
						if(item!=null) {
							activeChar.destroyItem("Consume", item.getObjectId(), 1, null, false);
						}
					}
					*/
					if ((skill.getId() != 2037 && skill.getId() != 2005 && (skill.getId() != 2166 && skill.getLevel() == 2)) && skill.isPotion() && activeChar instanceof L2PlayableInstance) {
						Potions.delete_Potion_Item(activeChar, skill.getId(), skill.getLevel());
					}
