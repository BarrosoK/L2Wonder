...
public class AdminCreateItem implements IAdminCommandHandler
{
	...
	private void massCreateItem(final L2PcInstance activeChar, final int id, final int num)
	{
		...
		{
			if (player.getClient().getConnection() == null ||  addresses.contains(player.getClient().getConnection().getInetAddress()) ) {
				continue;
			}
			...
			addresses.add(player.getClient().getConnection().getInetAddress());
			i++;
		}
