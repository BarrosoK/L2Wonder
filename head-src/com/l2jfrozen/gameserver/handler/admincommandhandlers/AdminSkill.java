...
public class AdminSkill implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(final String command, final L2PcInstance activeChar)
	{
		...
		}else if(command.startsWith("admin_add_all_clan_skills")){
			adminAddAllClanSkills(activeChar);
		}
	...
	private void adminAddClanSkill(final L2PcInstance activeChar, final int id, final int level)
	{
		...
		{
			AdminHelpPage.showHelpPage(activeChar, "custom_clan.htm");
			activeChar.sendPacket(new SystemMessage(SystemMessageId.INCORRECT_TARGET));
	...
	private void adminAddAllClanSkills(final L2PcInstance activeChar)
	{
		L2Object target = activeChar.getTarget();
		L2PcInstance player = null;
		...
		for(int id=370;id<391;id++) {
			...
			if (skill != null) {
				...
				player.getClan().broadcastToOnlineMembers(new PledgeSkillList(activeChar.getClan()));
			}
		}
