...
import static com.l2jfrozen.Config.ADFILTER_LIST;
...
public class Shutdown extends Thread {
    ...
    private void countdown() {
        ...
        try {
            while (_secondsShut > 0) {
                ...
                if ((_seconds <= 20 || _seconds % 60 == 0) && _seconds <= 600 && _hours <= 1) {
    ...
    private synchronized void saveData() {
        ...
        VoteRewardData.getInstance().store();
        try {
            wait(5000);
        } catch (final InterruptedException e1) {
        }

        try (BufferedWriter bw = new BufferedWriter(new FileWriter(FService.ADFILTER_FILE,false))) {

            PrintWriter pw = new PrintWriter(bw);

            for (int i = 0; i < ADFILTER_LIST.size(); i++) {
                pw.println(ADFILTER_LIST.get(i));
            }
            pw.close();
            wait(500);
