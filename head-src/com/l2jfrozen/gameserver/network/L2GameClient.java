...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>> implements Runnable {
    ...
    public void onForcedDisconnection(final boolean critical) {
        ...
        if (this.activeChar != null) {
            ...
            if (this.activeChar.isInParty() && this.activeChar.getParty() != null && this.activeChar.getParty().getLeader() != null) {
                if (L2ArenaManagerInstance.participantsLobby.contains(this.activeChar.getParty()) && this.activeChar == this.activeChar.getParty().getLeader()) {
                    L2ArenaManagerInstance.participantsLobby.remove(this.activeChar.getParty());
                }
            }
                ...
                for (ArenaFight a : L2ArenaManagerInstance.fights) {
                    if (a != null) {
                        for (L2PcInstance p : a.getFighters()) {
                            if (p.getObjectId() == this.activeChar.getObjectId()) {
                                a.endGame(p);
                                break;
                            }
                        }
...
/*
		L2PcInstance player = this.activeChar;

		if (player.isInDiceEvent() && player.getDiceEvent() != null){
			player.getDiceEvent().removePlayer(player);
		}
*/
            ...
            try {
                ...
                if (player != null) // this should only happen on connection loss
                {
                    ...
                    if (player.atEvent) {
                        ...
                        } else if (player._inEventBR) {
                            BattleRoyale.onDisconnect(player);
        ...
        public void run() {
            try {
                ...
                if (player != null) // this should only happen on connection loss
                {
                    ...
                    if (player.isInParty() && player.getParty() != null && player.getParty().getLeader() != null) {
                        if (L2ArenaManagerInstance.participantsLobby.contains(player.getParty()) && player == player.getParty().getLeader()) {
                            L2ArenaManagerInstance.participantsLobby.remove(player.getParty());
                        }
                    }
