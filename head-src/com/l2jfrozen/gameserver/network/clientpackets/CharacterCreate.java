...
public final class CharacterCreate extends L2GameClientPacket
{
	...
	private void initNewChar(final L2GameClient client, final L2PcInstance newChar)
	{
		...
		L2ItemInstance itemSpawn = null;

		if (newChar.isMageClass())
			{
				int[][] mageStartingItems = {
						{57, 2000},//adena
						{889, 1},//tate ring
						{2407, 1},//dc robe
						{5767, 1},//dc gloves
				...
				for (int z = 0; z < mageStartingItems.length; z++) {

					if (ItemTable.getInstance().createDummyItem(mageStartingItems[z][0]).isStackable())
						itemSpawn =	newChar.getInventory().addItem("Starter Items Mage", mageStartingItems[z][0], mageStartingItems[z][1], newChar, null);
					else
						for (int i = 0; i < mageStartingItems[z][1]; ++i)
							itemSpawn =	newChar.getInventory().addItem("Starter Items Mage", mageStartingItems[z][0], 1, newChar, null);

					if (itemSpawn.isEquipable())
					newChar.getInventory().equipItemAndRecord(itemSpawn);

				}
			}
			else
			{
				int[][] fighterStartingItems = {
						{57, 2000},//adena
						{728, 50},//mana pots
						{889, 1},//tate ring
						{889, 1},//tate ring
				...
				for (int z = 0; z < fighterStartingItems.length; z++) {
					if (ItemTable.getInstance().createDummyItem(fighterStartingItems[z][0]).isStackable())
						itemSpawn = newChar.getInventory().addItem("Starter Items Fighter", fighterStartingItems[z][0], fighterStartingItems[z][1], newChar, null);
					else
						for (int i = 0; i < fighterStartingItems[z][1]; ++i)
							itemSpawn =	newChar.getInventory().addItem("Starter Items Fighter", fighterStartingItems[z][0], 1, newChar, null);
