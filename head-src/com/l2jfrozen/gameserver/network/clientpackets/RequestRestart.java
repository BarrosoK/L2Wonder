	...
	protected void runImpl()
	{
		...
		if (player.getOlympiadGameId() > 0 || player.isInOlympiadMode() || Olympiad.getInstance().isRegistered(player))
		{
			...
			return;
		              ...
		              if(player.isInsideMultifunctionZone && !L2MultiFunctionZone.restart_zone)
		              {
		...
		if (L2ArenaManagerInstance.inFightOrWaiting.contains(player) ){
			player.sendMessage("You can't Restart in Street Fight arena.");
			return;
		}
		...
		// Check if player is in EventRT
		if (player._inEventCTF || player._inEventDM || player._inEventTvT || player._inEventVIP || player._inEventBR)
		{
		...
		if (player.isInParty() && (player==player.getParty().getLeader()||player.getParty().getPartyMembers().size()<=2) && L2ArenaManagerInstance.participantsLobby.contains(player.getParty()) ){
