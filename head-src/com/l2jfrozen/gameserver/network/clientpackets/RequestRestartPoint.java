...
import com.l2jfrozen.gameserver.managers.PvpZoneChangeManager;
...
public final class RequestRestartPoint extends L2GameClientPacket {
            ...
            try {
                ...
                switch (_requestedPointType) {
                    ...
                    default:
                        ...
                        if (activeChar.isInsideMultifunctionZone) {
                            PvpZonesData.getInstance().pvpZones.get(PvpZoneChangeManager.getInstance().currentZone).getSpawnLocs().size();
                            Random rand = new Random();
                            int n = rand.nextInt(PvpZonesData.getInstance().pvpZones.get(PvpZoneChangeManager.getInstance().currentZone).getSpawnLocs().size());

                            loc = PvpZonesData.getInstance().pvpZones.get(PvpZoneChangeManager.getInstance().currentZone).getSpawnLocs().get(n);
                            break;
                        }

                        if (activeChar.isInsideZone(L2Character.ZONE_PVP) || activeChar.isInsideZone(L2Character.ZONE_SIEGE) || activeChar.isInFunEvent()) {
                            loc = MapRegionTable.getInstance().getTeleToLocation(activeChar, MapRegionTable.TeleportWhereType.Town);
                            break;
                        }

                        loc = new Location(81310, 148604, -3495);
    ...
    protected void runImpl() {
        ...
        if (activeChar.isInsideBossZone) {
            ThreadPoolManager.getInstance().scheduleGeneral(new DeathTask(activeChar), 15 * 1000);
            activeChar.sendMessage("You will be teleported back in 15 seconds.");
            return;
        }
