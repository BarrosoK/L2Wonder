...
public final class RequestJoinParty extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (L2ArenaManagerInstance.inFightOrWaiting.contains(requestor) || L2ArenaManagerInstance.inFightOrWaiting.contains(target)) {
            requestor.sendMessage("You can't invite that player in party: you or your target is in Street Fight Arena.");
            return;
        }
        if (target._active_boxes > 1) {
            if (L2ArenaManagerInstance.participantsLobby.contains(requestor.getParty())) {
                for (L2Party party : L2ArenaManagerInstance.participantsLobby) {
                    final List<String> players_in_boxes = target.active_boxes_characters;
                    if (players_in_boxes != null && players_in_boxes.size() > 1) {
                        for (final String character_name : players_in_boxes) {
                            ...
                            if (p2 != null && party.getPartyMembers().contains(p2)) {
                                requestor.sendMessage("You cannot invite that player to the party because he is already registered with another character.");
                                return;
                            }
                        }
                    }
