...
public final class RequestWithDrawalParty extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (party != null) {
            ...
            for (ArenaFightPtVsPt a : L2ArenaManagerInstance.fightPtVsPt) {
                if (a != null) {
                    for (L2PcInstance p : a.getFighters()) {
                        if (p == player) {
                            player.sendMessage("You can't exit party when you are in Party vs Party arena.");
                            return;
                        }
                    }
                }
            ...
            if (player.isInsidePartyZone) {

                player.isInsidePartyZone = false;
                int playersInPartyZone = 0;
                for (L2PcInstance p : player.getParty().getPartyMembers()) {
                    if (p.isInsidePartyZone && p != player) {
                        playersInPartyZone++;
                    }
                }
                if (playersInPartyZone < 4) {
                    /*if (L2PartyPvPZone.partiesInside.containsKey(pt)) {
                        L2PartyPvPZone.partiesInside.remove(pt);
                    }
                    */
                    for (L2PcInstance p : player.getParty().getPartyMembers()) {
                        if (p == null) {
                        ...
                        if (p.isInsidePartyZone) {

                            p.teleToLocation(112688, 84544, -6536, true);
                            p.sendMessage("You are no longer 4 party members inside the party zone.");
                            p.isInsidePartyZone = false;
                        }
                    }

                }
                player.teleToLocation(112688, 84544, -6536, true);
