...
class OlympiadGame {
    ...
    protected OlympiadGame(final int id, final COMP_TYPE type, final L2FastList<L2PcInstance> list) {
        ...
        if (list != null && !L2ArenaManagerInstance.inFightOrWaiting.contains(list.get(0)) && !L2ArenaManagerInstance.inFightOrWaiting.contains(list.get(1))
                && (!list.get(0).isInParty()||(list.get(0).isInParty() && !L2ArenaManagerInstance.participantsLobby.contains(list.get(0).getParty()))) && (!list.get(1).isInParty()||(list.get(1).isInParty() && !L2ArenaManagerInstance.participantsLobby.contains(list.get(1).getParty())))
                && !L2ArenaManagerInstance.participants.contains(list.get(0)) && !L2ArenaManagerInstance.participants.contains(list.get(1))
                )
