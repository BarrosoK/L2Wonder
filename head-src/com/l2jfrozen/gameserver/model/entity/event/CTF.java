...
public class CTF implements EventTask {
    ...
    public static boolean startJoin() {
        ...
        Announcements.getInstance().gameAnnounceToAll(_eventName + ": Press .ctfjoin to participate.");
    ...
    public static boolean startTeleport() {
        ...
        if (_teamEvent) {
            ...
            if (!checkMinPlayers(_players.size())) {
                Announcements.getInstance().gameAnnounceToAll(_eventName + ": Not enough players for event.\nMin Requested : " + _minPlayers + ", Participating : " + _players.size());
                if (Config.CTF_STATS_LOGGER)
                            ...
                            if (_teamEvent) {
                                ...
                                try {
                                    ...
                                    switch (player._teamNameCTF) {
                                        case "Blue": {
                                            player.teleToLocation(_teamsX.get(0) + Rnd.get(offset), _teamsY.get(0) + Rnd.get(offset), _teamsZ.get(0));
                                            break;
                                        }
                                        case "Red": {
                                            player.teleToLocation(_teamsX.get(1) + Rnd.get(offset), _teamsY.get(1) + Rnd.get(offset), _teamsZ.get(1));
                                            break;
                                        }
                                        default: {
                                            player.teleToLocation(_teamsX.get(0) + Rnd.get(offset), _teamsY.get(0) + Rnd.get(offset), _teamsZ.get(0));
                                    ...
                                    LOGGER.info("ctf out of bounds" + CTF._teams.indexOf(player._teamNameCTF) + " " + CTF._teamsX.size() + " " + CTF._teamsY.size() + " " + CTF._teamsZ.size());
                                }
            ...
            for (final L2PcInstance player : _players) {
                if (player != null && (player.isOnline() != 0) && (player._inEventCTF)) {
                    if (teamName != null && (player._teamNameCTF.equals(teamName))) {
                        ...
                        replyMSG.append("<html><body>");
                        replyMSG.append("<font color=\"FFFF00\">Your team had a tie in the event. You didn't receive any rewards.</font>");
                        replyMSG.append("</body></html>");
                        ...
                        replyMSG.append("<html><body>");
                        replyMSG.append("<font color=\"FFFF00\">Your team lost the event. You didn't receive any rewards.</font>");
                        replyMSG.append("</body></html>");
