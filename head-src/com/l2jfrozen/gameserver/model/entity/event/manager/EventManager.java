...
public class EventManager
{
	...
	public static boolean BR_EVENT_ENABLED;
	public static ArrayList<String> BR_TIMES_LIST;
	...
	public static void loadConfiguration()
	{
		...
		try
		{
			...
			BR_EVENT_ENABLED = Boolean.parseBoolean(eventSettings.getProperty("BREventEnabled", "false"));
			BR_TIMES_LIST = new ArrayList<>();

			propertySplit = eventSettings.getProperty("BRStartTime", "").split(";");

			for (final String time : propertySplit)
			{
				BR_TIMES_LIST.add(time);
			}
	...
	private static void registerBR()
	{
		BattleRoyale.loadData();
		if (!BattleRoyale.checkStartJoinOk())
		{
			LOGGER.error("registerBR: BR Event is not setted Properly");
		}

		// clear all tvt
		EventsGlobalTask.getInstance().clearEventTasksByEventName(BattleRoyale.get_eventName());

		for (final String time : BR_TIMES_LIST)
		{

			final BattleRoyale newInstance = BattleRoyale.getNewInstance();
			// LOGGER.info("registerDM: reg.time: "+time);
			newInstance.setEventStartTime(time);
			EventsGlobalTask.getInstance().registerNewEventTask(newInstance);
