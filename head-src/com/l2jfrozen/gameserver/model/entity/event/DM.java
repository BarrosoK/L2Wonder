...
public class DM implements EventTask {
    ...
    public static boolean startJoin() {
        ...
        Announcements.getInstance().gameAnnounceToAll(_eventName + ": Press .dmjoin to participate.");
    ...
    public static void finishEvent() {
        ...
        if (_teamEvent) {
            ...
            if (_topKills != 0) {
                ...
                if (_topPlayers.size() > 1) {
                    Announcements.getInstance().gameAnnounceToAll(_eventName + ": " + winners + " win the match! There are "+_topPlayers.size()+" winners with "+ _topKills +" kills and they will share the reward.");
                } else {
    ...
    private static boolean addPlayerOk(final L2PcInstance eventPlayer) {
        ...
        if (!Config.DM_ALLOW_HEALER_CLASSES && (eventPlayer.getClassId() == ClassId.cardinal || eventPlayer.getClassId() == ClassId.evaSaint || eventPlayer.getClassId() == ClassId.shillienSaint)) {
            eventPlayer.sendMessage("You cant join with a Healer Class!");
            return false;
        }
    ...
    public static void rewardPlayer() {
        if (_topPlayers.size() > 0) {
            ...
            for (final L2PcInstance _topPlayer : _topPlayers) {
                if (_topPlayers.size() > _rewardAmount) {
                    _topPlayer.addItem("DM Event: " + _eventName, _rewardId, 1, _topPlayer, true);
                } else {
                    _topPlayer.addItem("DM Event: " + _eventName, _rewardId, (int) (_rewardAmount / _topPlayers.size()), _topPlayer, true);
                }
                ...
                if (_topPlayers.size() > _rewardAmount) {
                    replyMSG.append("<html><body>You won the event with " + _topPlayers.size() + " other players. You will split the reward. Look in your inventory for it.</body></html>");
                } else {
