...
public class TvT implements EventTask
{
	...
	public static boolean startJoin()
	{
		...
		Announcements.getInstance().gameAnnounceToAll(_eventName + ": Reward: " + _rewardAmount + " " + ItemTable.getInstance().getTemplate(_rewardId).getName()+" ("+_rewardAmount*2+" for Top killer)");
		Announcements.getInstance().gameAnnounceToAll(_eventName + ": Recruiting levels: " + _minlvl + " to " + _maxlvl);
		Announcements.getInstance().gameAnnounceToAll(_eventName + ": Press .tvtjoin to participate.");
	...
	public static boolean startTeleport()
	{
		...
		else if (Config.TVT_EVEN_TEAMS.equals("SHUFFLE") && !checkMinPlayers(_playersShuffle.size()))
		{
			...
			return false;
		}else if (Config.TVT_EVEN_TEAMS.equals("BALANCE") && !checkMinPlayers(_players.size()))
		{
			Announcements.getInstance().gameAnnounceToAll(_eventName + ": Not enough players for event.\nMin Requested : " + _minPlayers + ", Participating : " + _playersShuffle.size());
			if (Config.CTF_STATS_LOGGER)
				LOGGER.info(_eventName + ":Not enough players for event. Min Requested : " + _minPlayers + ", Participating : " + _playersShuffle.size());
		...
		ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
		{
			...
			public void run()
			{
				...
				for(L2ArenaZone a : ArenaManager.getInstance().getArenas()){
					if(a.getId()==11012){
						a.removeAllOthers(_players);
						break;
					}
				}
			...
			synchronized (_players)
			{
				...
				if (_topKills != 0)
				{
					...
					if (Config.TVT_ANNOUNCE_TEAM_STATS)
					{
						...
						{
							Announcements.getInstance().gameAnnounceToAll(_eventName + ": Top loser: " + looser.getName() + " - Deaths: " + looser._countTvTdies);
						}
					}
					...
					if (Config.TVT_STATS_LOGGER)
					{
						...
						if ((looser != null) && (!looser.equals(bestKiller)))
						{
							LOGGER.info("Top looser: " + looser.getName() + " - Deaths: " + looser._countTvTdies);
						}
	...
	private static boolean addPlayerOk(final String teamName, final L2PcInstance eventPlayer)
	{
		...
		{
			eventPlayer.sendMessage("You already participated in street fight arena!");
			return false;
		}
	...
	public static void rewardTeam(final String teamName, final L2PcInstance bestKiller, final L2PcInstance looser)
	{
		...
		synchronized (_players)
		{
			...
			for (final L2PcInstance player : _players)
			{
				...
				if (player != null && (player.isOnline() != 0) && (player._inEventTvT))// && (player._countTvTkills > 0 || Config.TVT_PRICE_NO_KILLS))
				{

					if ((bestKiller != null) && (bestKiller.equals(player)))
					{
						player.addItem("Event Reward", _rewardId, _rewardAmount*2, player, true);
						final NpcHtmlMessage nhm = new NpcHtmlMessage(5);
						final TextBuilder replyMSG = new TextBuilder("");

						replyMSG.append("<html><body>");
						replyMSG.append("<font color=\"FFFF00\">Your team wins the event and you are the top killer. <br>Look in your inventory for the reward.</font>");
						replyMSG.append("</body></html>");
					...
					{
						player.addItem("Event Reward", _rewardId, _rewardAmount, player, true);
					...
					{ // TIE
						...
						player.addItem("Event Reward", _rewardId, minus_reward, player, true);
						final NpcHtmlMessage nhm = new NpcHtmlMessage(5);
						final TextBuilder replyMSG = new TextBuilder("");
						...
						replyMSG.append("<html><body>");
						replyMSG.append("<font color=\"FFFF00\">Your team lost the event.</font>");
						replyMSG.append("</body></html>");
