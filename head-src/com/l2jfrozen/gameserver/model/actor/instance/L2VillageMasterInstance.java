...
public final class L2VillageMasterInstance extends L2FolkInstance {
    ...
    public void onBypassFeedback(final L2PcInstance player, final String command) {
        ...
        if (actualCommand.equalsIgnoreCase("create_clan")) {
            ...
            player.sendMessage("You can't create academy in this server.");
            return;
        ...
        } else if (actualCommand.equalsIgnoreCase("custom_learn_all_clan_skills")) {
            if (player.getClan() == null) {
                ...
                int membersCount = 0;
                ArrayList<String> membersIps = new ArrayList<>();
                for (L2PcInstance p : player.getClan().getOnlineMembers("")) {

                    if (p.getClient() != null && p.getClient().getConnection() != null && !p.getClient().getConnection().isClosed() && p.getClient().getConnection().getInetAddress() != null) {
                        if (!membersIps.contains(p.getClient().getConnection().getInetAddress().getHostAddress())) {
                            membersCount++;
                        }
                        membersIps.add(p.getClient().getConnection().getInetAddress().getHostAddress());
                    }
                }
                ...
                if (membersCount >= 10) {
                    if (player.getClan().getLevel() < 8) {
                        ...
                        player.sendMessage("You have " + membersCount + " unique players online. You were given clan lvl 8.");
                    } else {
                        player.sendMessage("You have " + membersCount + " unique players online. Your clan is already level 8.");
                    }
                    if (membersCount >= 20) {
                        if (!player.getClan().gotFullSkills) {
                            ...
                            player.sendMessage("You have " + membersCount + " unique players online. You were given full clan skills.");
                            player.getClan().gotFullSkills = true;
                        }else{
                            player.sendMessage("You have " + membersCount + " unique players online. You already received full clan skills.");
                        }
                    }

                } else {
                    player.sendMessage("You have only " + membersCount + " unique players online. You did not get a reward.");
                }
            ...
            if (player.isCursedWeaponEquiped()) {
                player.sendMessage("You can`t change Subclass while Cursed weapon equipped!");
                return;
            }
            ...
            switch (cmdChoice) {
                ...
                case 4: // Add Subclass - Action (Subclass 4 x[x])
                    ...
                    if (player._inEventTvT || player._inEventCTF || player._inEventDM || player._inEventBR ) {
                    ...
                    // You can't add Subclass when you are registered in Events (TVT, CTF, DM)
                    if (L2ArenaManagerInstance.inFightOrWaiting.contains(player)||L2ArenaManagerInstance.participants.contains(player)) {
                        player.sendMessage("You can't add a subclass registered in arena.");
                        player.setLocked(false);
                        return;
                    }
                ...
                case 5: // Change Class - Action
                    ...
                    if (L2ArenaManagerInstance.participants.contains(player)) {
                        player.sendMessage("You cannot change subclass while registered in street fight arena.");
                        player.setLocked(false);
                        return;
                    }
    ...
    private final Set<PlayerClass> getAvailableSubClasses(final L2PcInstance player) {
        ...
        if (availSubs != null) {
            for (final PlayerClass availSub : availSubs) {
                ...
                if (player.getBaseClass() >= 88) {
                    baseClass = ClassId.values()[baseClass].getParent().getId();
                }

                if (baseClass == availSub.ordinal()) {
