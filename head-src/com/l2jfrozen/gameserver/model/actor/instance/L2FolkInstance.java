...
public class L2FolkInstance extends L2NpcInstance {
    ...
    public void showSkillList(final L2PcInstance player, final ClassId classId) {
        ...
        if (!getTemplate().canTeach(classId) && npcId != 100 && !(this instanceof L2DonateShopInstance)) {
            NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
            TextBuilder sb = new TextBuilder();
            sb.append("<html><body>");
            sb.append("Nique ta mere I cannot teach you any skills.<br> You must find your current class teachers.");
            sb.append("</body></html>");
            html.setHtml(sb.toString());
            player.sendPacket(html);
            html = null;
    ...
    public void showEnchantSkillList(final L2PcInstance player, final ClassId classId) {
        ...
        for (final L2EnchantSkillLearn s : skills) {
            ...
            if (sk == null ||( s.getLevel() == 154 &&player.getLastFolkNPC() instanceof L2DonateShopInstance)
                    || (s.getLevel() == 155 && player.getLastFolkNPC() instanceof L2DonateShopInstance)
                    ||( s.getLevel() == 114 && player.getLastFolkNPC() instanceof L2DonateShopInstance)
                    || (s.getLevel() == 115 && player.getLastFolkNPC() instanceof L2DonateShopInstance)) {
                continue;
            }
            counts++;
