...
/**
 * Created by Server1 on 8/21/2017.
 */
...
public class BotsPreventionManager {
    ...
    public static Map<Integer, Integer> _monsterscounter;
    ...
    public void updatecounter(L2Character player, L2Character monster) {
        ...
        if ((player instanceof L2PcInstance) && (monster instanceof L2MonsterInstance)) {
            ...
            if ((Config.KILLS_COUNTER * ((L2PcInstance) player).bot_prevention_counter) + next < count) {
    ...
    public void punishmentnwindow(L2PcInstance player) {
        ...
        StringUtil.append(tb, "<title>Bots prevention</title>");
        StringUtil.append(tb, "<body><center><br><br><img src=\"L2UI_CH3.herotower_deco\" width=\"256\" height=\"32\">");
        StringUtil.append(tb, "<br><br><font color=\"a2a0a2\">If such window appears, it means you have 60 seconds<br1>to answer correctly.");
        StringUtil.append(tb, "<br><br><font color=\"b09979\">in case character gets the wrong answer he will be kicked.</font>");
        StringUtil.append(tb, "</center></body>");
        StringUtil.append(tb, "</html>");
        html.setHtml(tb.toString());
        player.sendPacket(html);
    ...
    protected void banpunishment(L2PcInstance player) {
        ...
        switch (Config.PUNISHMENT) {
            ...
            case 0:
                ...
                player.teleToLocation(81310, 148604, -3495);
                player.sendMessage("Unfortunately, colours don't match, you were teleported to Giran. Punishments will increase and possible bans, don't use bots.");
        ...
        //player.sendMessage("Unfortunately, colours don't match. You are jailed for 10 minutes. Punishment times will increase and possible bans, don't use bots");
        player.bot_prevention_counter = 1;
    }
    ...
    public void AnalyseBypass(String command, L2PcInstance player) {
        ...
        if (choosenoption > -1) {
            ...
            if (choosenoption != playerData.mainpattern) {
                ...
                if (player.bot_prevention_counter < 8) {
                    player.bot_prevention_counter++;
                }
                _validation.remove(player.getObjectId());
                _beginvalidation.get(player.getObjectId()).cancel(true);
                _beginvalidation.remove(player.getObjectId());
