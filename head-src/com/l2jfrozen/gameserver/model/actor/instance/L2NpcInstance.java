...
public class L2NpcInstance extends L2Character {
    ...
    public void onActionShift(final L2GameClient client) {
        ...
        if (player.getAccessLevel().isGm()) {
            ...
            if (this instanceof L2MonsterInstance || this instanceof L2GrandBossInstance || this instanceof L2RaidBossInstance) {
                // Set the target of the L2PcInstance player
                player.setTarget(this);
                ...
                html1.append("<table border=0 width=\"98%\">");
                ...
                for (final L2DropCategory cat : getTemplate().getDropData()) {
                    ...
                    if (drops != null)
                        for (final L2DropData drop : drops) {
                            ...
                            if (drop.getChance() < 100000) {
                                html1.append("<tr><td>" + name + "</td><td>" + "<font color=\"ee2114\">Low Rate</font>" + "</td></tr>");
                            } else if (drop.getChance() >= 400000) {
                                html1.append("<tr><td>" + name + "</td><td>" + "<font color=\"51bd22\">High Rate</font>" + "</td></tr>");
                            } else {
                                html1.append("<tr><td>" + name + "</td><td>" + "<font color=\"LEVEL\">Medium Rate</font>" + "</td></tr>");
                            }
                        }
                }
    ...
    private void showQuestChooseWindow(final L2PcInstance player, final Quest[] quests) {
        if (this.getNpcId() == 1234321) {
            showCustomQuestChooseWindow(player, quests);
        } else {
    ...
    private void showCustomQuestChooseWindow(final L2PcInstance player, final Quest[] quests) {
        final TextBuilder sb = new TextBuilder();

        sb.append("<html><body><title>Quest Manager</title><br>");

        sb.append("<table width=300 border=0><tr><td width=270 height=100><img src=\"l2wonderlust.logo\" width=275 height=100 align=center></td></table><body><img src=\"L2UI.SquareGray\" width=300 height=1><br><br>");
        ...
        for (final Quest q : quests) {
            ...
            sb.append("<center><button value=\"" + q.getDescr() + state + "\" action=\"bypass -h npc_" + getObjectId() + "_Quest " + q.getName() + "\"" + " width=280 height=25 back=\"l2wonderlust.active\" fore=\"l2wonderlust.passive\"></center><br>");
