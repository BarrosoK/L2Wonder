...
import com.l2jfrozen.gameserver.custom.PvpZoneInstance;
...
public final class L2PcInstance extends L2PlayableInstance {
    ...
    private long _lastHopVote = 0;
    private long _lastTopVote = 0;
    private long _lastNetVote = 0;
    ...
    public boolean _isInDiceEvent = false;
    public L2DiceEventInstance diceEvent = null;

    public L2DiceEventInstance getDiceEvent() {
        return diceEvent;
    }

    public void setDiceEvent(L2DiceEventInstance event) {

        diceEvent = event;
    }

    public boolean isInDiceEvent() {
        return _isInDiceEvent;
    }

    public void setIsInDiceEvent(boolean bool) {
        _isInDiceEvent = bool;
    }
     ...
     */
    private static final String UPDATE_CHARACTER = "UPDATE characters SET level=?,maxHp=?,curHp=?,maxCp=?,curCp=?,maxMp=?,curMp=?,str=?,con=?,dex=?,_int=?,men=?,wit=?,face=?,hairStyle=?,hairColor=?,sex=?,heading=?,x=?,y=?,z=?,exp=?,expBeforeDeath=?,sp=?,karma=?,pvpkills=?,pkkills=?,rec_have=?,rec_left=?,clanid=?,maxload=?,race=?,classid=?,deletetime=?,title=?,accesslevel=?,online=?,isin7sdungeon=?,clan_privs=?,wantspeace=?,base_class=?,onlinetime=?,punish_level=?,punish_timer=?,newbie=?,nobless=?,power_grade=?,subpledge=?,last_recom_date=?,lvl_joined_academy=?,apprentice=?,sponsor=?,varka_ketra_ally=?,clan_join_expiry_time=?,clan_create_expiry_time=?,char_name=?,death_penalty_level=?,pc_point=?,name_color=?,title_color=?,aio=?,aio_end=?,daypvp=?,pvpmaster=? WHERE obj_id=?";
     ...
     * ,
     * banchat_time
     */",name_color,title_color,first_log,aio,aio_end,daypvp,pvpmaster,last_hop_vote,last_top_vote,last_net_vote FROM characters WHERE obj_id=?";
    ...
    public class AIAccessor extends L2Character.AIAccessor {
        ...
        public void doAttack(final L2Character target) {
            ...
            if ((TvT.is_teleport() && _inEventTvT) || (CTF.is_teleport() && _inEventCTF) || (DM.is_teleport() && _inEventDM) || (BattleRoyale.is_teleport() && _inEventBR)) {
                sendPacket(ActionFailed.STATIC_PACKET);
                return;
            }
    ...
    public boolean _inEventDMTeleported = false;

    public boolean _inEventBR = false;

    public int bot_prevention_counter = 1;
    ...
    public ArrayList<Integer> playersReported = new ArrayList<>();

    public boolean isInsideWaitingRoom = false;

    public boolean isInsideMultifunctionZone = false;

    public boolean isInsideBossZone = false;

    public boolean isInsidePartyZone = false;

    public boolean isInsideCustomPartyArenaZone = false;

    public boolean isInsideColiseumZone = false;

    public boolean isInsideBattleRoyaleZone = false;

    public boolean isInsideStoreZone = false;

    public boolean isEligibleForRankingsReward = false;


    public boolean PartyZone = false;
                ...
                if (object instanceof L2NpcInstance && isInsideRadius(object, L2NpcInstance.INTERACTION_DISTANCE, false, false)) {
                    ...
                    if (states != null) {
                        for (final QuestState state : states) {
                            if (state != null && qs != null && state.getQuest().getQuestIntId() == qs.getQuest().getQuestIntId() && !qs.isCompleted()) {
    ...
    public boolean useAutoHPPots = false;

    private Map<Integer, Future<?>> _autoHPTasks = new HashMap<>();

    public boolean isAutoHPPot(int id) {
        return _autoHPTasks.keySet().contains(id);
    }

    public void setAutoHPPot(int id, Future<?> task, boolean add) {
        if (add)
            _autoHPTasks.put(id, task);
        else {
            if (_autoHPTasks.get(id) != null) {
                _autoHPTasks.get(id).cancel(false);
            }
            _autoHPTasks.remove(id);
        }
    }

    public boolean useAutoMPPots = false;

    private Map<Integer, Future<?>> _autoMPTasks = new HashMap<>();

    public boolean isAutoMPPot(int id) {
        return _autoMPTasks.keySet().contains(id);
    }

    public void setAutoMPPot(int id, Future<?> task, boolean add) {
        if (add)
            _autoMPTasks.put(id, task);
        else {
            if (_autoMPTasks.get(id) != null) {
                _autoMPTasks.get(id).cancel(false);
            }
            _autoMPTasks.remove(id);
        }
    }

    public boolean useAutoCPPots = false;

    private Map<Integer, Future<?>> _autoCPTasks = new HashMap<>();

    public boolean isAutoCPPot(int id) {
        return _autoCPTasks.keySet().contains(id);
    }

    public void setAutoCPPot(int id, Future<?> task, boolean add) {
        if (add)
            _autoCPTasks.put(id, task);
        else {
            if (_autoCPTasks.get(id) != null) {
                _autoCPTasks.get(id).cancel(false);
            }
            _autoCPTasks.remove(id);
        }
    }
    ...
    public void refreshMasteryWeapPenality() {
        ...
        if (!_bow_mastery && !_blunt_mastery && !_dagger_mastery && !_fist_mastery && !_dual_mastery && !_pole_mastery && !_sword_mastery && !_2hands_mastery) { // not completed 1st class transfer or not acquired yet the mastery skills
            newMasteryPenalty = 0;
        } else {
            for (final L2ItemInstance item : getInventory().getItems()) {
    ...
    public void onAction(final L2PcInstance player) {
        ...
        if (((TvT.is_started() || TvT.is_teleport()) && !Config.TVT_ALLOW_INTERFERENCE) || ((CTF.is_started() || CTF.is_teleport()) && !Config.CTF_ALLOW_INTERFERENCE) || ((DM.is_started() || DM.is_teleport()) && !Config.DM_ALLOW_INTERFERENCE) || ((BattleRoyale.is_started() || BattleRoyale.is_teleport()) && !Config.BR_ALLOW_INTERFERENCE)) {
            if ((_inEventTvT && !player._inEventTvT) || (!_inEventTvT && player._inEventTvT)) {
                ...
                player.sendPacket(ActionFailed.STATIC_PACKET);
                return;
            } else if ((_inEventBR && !player._inEventBR) || (!_inEventBR && player._inEventBR)) {
                player.sendPacket(ActionFailed.STATIC_PACKET);
                return;
            }
        }
    ...
    public boolean isInFunEvent() {
        return (atEvent || isInStartedTVTEvent() || isInStartedDMEvent() || isInStartedBREvent() || isInStartedCTFEvent() || isInStartedVIPEvent());
    }
    ...
    public boolean isInStartedBREvent() {
        return (BattleRoyale.is_started() && _inEventBR);
    }

    public boolean isRegisteredInBREvent() {
        return _inEventBR;
    }
    ...
    public boolean isRegisteredInFunEvent() {
        return (atEvent || (_inEventTvT) || (_inEventDM) || (_inEventCTF) || (_inEventVIP) || (_inEventBR) || Olympiad.getInstance().isRegistered(this));
    }
    ...
    public void updatePvPColor(final int pvpKillAmount) {
        if (Config.PVP_COLOR_SYSTEM_ENABLED) {
            ...
            if (pvpKillAmount < Config.PVP_AMOUNT1) {
                getAppearance().setNameColor(0xFFFFFF);
            } else if (pvpKillAmount >= Config.PVP_AMOUNT1 && pvpKillAmount < Config.PVP_AMOUNT2) {
    ...
    public void updatePkColor(final int pkKillAmount) {
        if (Config.PK_COLOR_SYSTEM_ENABLED) {
            ...
            if (pkKillAmount < Config.PK_AMOUNT1) {
                getAppearance().setTitleColor(0xFFFF77);
            } else if (pkKillAmount >= Config.PK_AMOUNT1 && pkKillAmount < Config.PVP_AMOUNT2) {
    ...
    public boolean doDie(final L2Character killer) {
        ...
        if (killer != null) {
            ...
            if (pk != null) {
                ...
                if (_inEventTvT && pk._inEventTvT) {
                        ...
                        ThreadPoolManager.getInstance().scheduleGeneral(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    switch (_teamNameCTF) {
                                        case "Blue": {
                                            teleToLocation(CTF._teamsX.get(0), CTF._teamsY.get(0), CTF._teamsZ.get(0), false);
                                            break;
                                        }
                                        case "Red": {
                                            teleToLocation(CTF._teamsX.get(1), CTF._teamsY.get(1), CTF._teamsZ.get(1), false);
                                            break;
                                        }
                                        default: {
                                            teleToLocation(CTF._teamsX.get(0), CTF._teamsY.get(0), CTF._teamsZ.get(0), false);
                                    ...
                                    LOGGER.info("ctf out of bounds" + CTF._teams.indexOf(_teamNameCTF) + " " + CTF._teamsX.size() + " " + CTF._teamsY.size() + " " + CTF._teamsZ.size());
                                }
                                doRevive();
                            }
                        }, 20000);
                ...
                } else if (_inEventBR && BattleRoyale.is_started()) {

                    Vector<L2PcInstance> toRemove = new Vector<>();
                    BattleRoyale.removePlayer(this);
                    broadcastUserInfo();
                    synchronized (BattleRoyale._players) {
                        for (final L2PcInstance player : BattleRoyale._players) {
                    ...
                    for (final L2PcInstance player : toRemove) {
                    ...
                    synchronized (BattleRoyale._players) {
                        for (final L2PcInstance player : BattleRoyale._players) {
                            if (player != null && player.isOnline() == 1) {
                                player.sendMessage("Battle Royale: " + BattleRoyale._players.size() + " players alive.");
                            }
                        }
                    }
            ...
            if (L2ArenaManagerInstance.inFightOrWaiting.contains(this) && L2ArenaManagerInstance.inFightOrWaiting.contains((killer))) {
                ArenaFight arena = null;
                for (ArenaFight a : L2ArenaManagerInstance.fights) {
                    for (L2PcInstance p : a.getFighters()) {
                        if (p.getCharId() == ((L2PcInstance) killer).getCharId() || p.getCharId() == this.getCharId()) {
                            arena = a;
                            break;
                        }
                    }
                ...
                if (arena != null) {
                    ...
                    ArenaFightPtVsPt ptArena = null;
                    for (ArenaFightPtVsPt a : L2ArenaManagerInstance.fightPtVsPt) {
                        if (a != null) {
                            if (a.getFighters() != null && a.getFighters().contains(this)) {
                                ptArena = a;
                                break;
                            }
                        }
            ...
            if (killer instanceof L2PcInstance) {
                ...
                if (!((L2PcInstance) killer).isGM()) {
                    this.sendPacket(new ExShowScreenMessage("Player: " + killer.getName() + " Has " + (int) killer.getCurrentHp() + " HP left.", 6000));
                }
/*
                if (this.isInsidePartyZone && ((L2PcInstance) killer).isInsidePartyZone) {
                    L2PartyPvPZone.pvpParty partyKiller = null;
                    L2PartyPvPZone.pvpParty partyVictim = null;

                    partyVictim = L2PartyPvPZone.partiesInside.get(this.getParty());
                    partyKiller = L2PartyPvPZone.partiesInside.get(killer.getParty());

                    if (partyKiller != null && partyVictim != null) {
                        partyKiller.addKill();
                        partyVictim.addDeath();
                    }
                }
                */
            }
    ...
    public void givePoints(L2PcInstance winner, L2PcInstance loser) {

        boolean allowPoints = true;

        for (HashMap.Entry<int[], Long> entry : L2ArenaManagerInstance.fighterIntervals.entrySet()) {
            if (entry.getKey()[0] == winner.getObjectId() && entry.getKey()[1] == loser.getObjectId()) {
                allowPoints = false;
                break;
            }
        }
        ...
        try {
            loser.setArenasInfos("arena_loses", loser.getArenaInfos("arena_loses") + 1);
        } catch (Exception e) {
            loser.setArenasInfos("arena_loses", loser.getArenaInfos("arena_loses") + 1);
        }
        try {
            loser.setArenasInfos("arena_points", loser.getArenaInfos("arena_points") - 1);
        } catch (Exception e) {
            loser.setArenasInfos("arena_points", loser.getArenaInfos("arena_points") - 1);
        }
        ...
        if (allowPoints) {
            ...
            try {
                winner.setArenasInfos("arena_wins", winner.getArenaInfos("arena_wins") + 1);
            } catch (Exception e) {
                winner.setArenasInfos("arena_wins", winner.getArenaInfos("arena_wins") + 1);
            }
            try {
                winner.setArenasInfos("arena_points", winner.getArenaInfos("arena_points") + 1);
            } catch (Exception e) {
                winner.setArenasInfos("arena_points", winner.getArenaInfos("arena_points") + 1);
            }
            L2ArenaManagerInstance.fighterIntervals.put(new int[]{winner.getObjectId(), loser.getObjectId()}, System.currentTimeMillis());

        } else {
            winner.sendMessage("You did not receive points for this fight. You can receive points against the same player every 10 minutes.");
        }
    }


    public int getArenaInfos(String value) {
        ...
        try {
            ...
            statment.setInt(1, this.getObjectId());
    ...
    public void setArenasInfos(String value, int amount) {
        ...
        try {
            ...
            statment.setInt(2, this.getObjectId());
            statment.execute();
            statment.close();
            con.close();
    ...
    private void onDieDropItem(final L2Character killer) {
        if (atEvent || (TvT.is_started() && _inEventTvT) || (DM.is_started() && _inEventDM) || (CTF.is_started() && _inEventCTF) || (VIP._started && _inEventVIP) || (BattleRoyale.is_started() && _inEventBR) || killer == null)
            return;
    ...
    public void onKillUpdatePvPKarma(final L2Character target) {
        ...
        if ((_inEventCTF && CTF.is_started()) || (_inEventTvT && TvT.is_started()) || (_inEventVIP && VIP._started) || (_inEventDM && DM.is_started()) || (_inEventBR && BattleRoyale.is_started()))
            return;
        ...
        }
        {
            ...
            if ((isInsideZone(ZONE_PVP) || targetPlayer.isInsideZone(ZONE_PVP)) && !(checkIfPvP(target) && targetPlayer.getPvpFlag() != 0)) {
                return;
            }
            // check anti-farm
            // possible pvp farming exploits in multifunction area
        ...
        if (checkIfPvP(target) && targetPlayer.getPvpFlag() != 0) {
            ...
            if (!(_inEventTvT && TvT.is_started()) || !(_inEventCTF && CTF.is_started()) || !(_inEventVIP && VIP._started) || !(_inEventDM && DM.is_started()) || !(_inEventBR && BattleRoyale.is_started())) {
    ...
    private boolean checkAntiFarm(final L2PcInstance targetPlayer) {
        ...
        if (Config.ANTI_FARM_ENABLED) {
            ...
            if (!checkTimeAntiFarm(targetPlayer)) {
                this.sendMessage("You can receive a kill point for killing the same player every 1 minute(s).");
                return false;
            }
    ...
    private boolean checkTimeAntiFarm(final L2PcInstance targetPlayer) {

        String killer = getName();
        String killed = targetPlayer.getName();
        long time = 0;
        Connection con = null;
        try {
            con = L2DatabaseFactory.getInstance().getConnection(false);
            final PreparedStatement statement = con.prepareStatement("SELECT time FROM pkkills WHERE killerId=? AND killedId=?");
        ...
        if (time != 0 && time + 60 * 1 * 1000 > System.currentTimeMillis()) {
            return false;
        }
    ...
    private void addItemReward(final L2PcInstance targetPlayer) {
        ...
        if (targetPlayer.getClient() != null && targetPlayer.getClient().getConnection() != null) {
            if (targetPlayer.getClient().getConnection().getInetAddress() != getClient().getConnection().getInetAddress()) {
                ...
                {
                    // if (isInsideMultifunctionZone && targetPlayer.isInsideMultifunctionZone) {
    ...
    public void increasePvpKills() {
        ...
        if ((TvT.is_started() && _inEventTvT) || (DM.is_started() && _inEventDM) || (CTF.is_started() && _inEventCTF) || (VIP._started && _inEventVIP) || (BattleRoyale.is_started() && _inEventBR))
            return;
    ...
    public void QuakeSystem() {
        ...
        switch (quakeSystem) {
            case 2:
                final CreatureSay cs0 = new CreatureSay(0, 15, "", getName() + " Double kill!"); // 8D
                for (final L2PcInstance player : L2World.getInstance().getAllPlayers()) {
                    if (player != null)
                        if (player.isOnline() != 0)
                            player.sendPacket(cs0);
                }
                break;
            case 3:
                final CreatureSay cs = new CreatureSay(0, 15, "", getName() + " Triple kill!"); // 8D
                for (final L2PcInstance player : L2World.getInstance().getAllPlayers()) {
                    if (player != null)
                        if (player.isOnline() != 0)
                ...
                break;
            case 6:
                final CreatureSay cs2 = new CreatureSay(0, 15, "", getName() + " Hexakill!!"); // 8D
                for (final L2PcInstance player : L2World.getInstance().getAllPlayers()) {
                    if (player != null)
                        if (player.isOnline() != 0)
            ...
            case 25:
            case 30:
                final CreatureSay cs3 = new CreatureSay(0, 15, "", getName() + " is GODLIKE with " + quakeSystem + " consecutive kills!!!"); // 8D
                for (final L2PcInstance player : L2World.getInstance().getAllPlayers()) {
                    if (player != null)
                        if (player.isOnline() != 0)
    ...
    public void doPkInfo(final L2PcInstance PlayerWhoKilled) {
        ...
        try {
            con = L2DatabaseFactory.getInstance().getConnection(false);
            final PreparedStatement statement = con.prepareStatement("SELECT kills,time FROM pkkills WHERE killerId=? AND killedId=?");
            statement.setString(1, killer);
            statement.setString(2, killed);
            final ResultSet rset = statement.executeQuery();
            if (rset.next()) {
        ...
        if (kills >= 1) {
            kills++;
            String UPDATE_PKKILLS = "UPDATE pkkills SET kills=?, time=? WHERE killerId=? AND killedId=?";
            ...
            try {
                ...
                statement.setString(3, killer);
                statement.setString(4, killed);
                statement.execute();
                DatabaseUtils.close(statement);
                statement = null;
                UPDATE_PKKILLS = null;
            ...
            String ADD_PKKILLS = "INSERT INTO pkkills (killerId,killedId,kills,time) VALUES (?,?,?,?)";
...
*/
    ...
    public void updatePvPStatus() {
        if ((TvT.is_started() && _inEventTvT) || (CTF.is_started() && _inEventCTF) || (DM.is_started() && _inEventDM) || (VIP._started && _inEventVIP) || (BattleRoyale.is_started() && _inEventBR))
            return;
    ...
    public void updatePvPStatus(final L2Character target) {
        ...
        if (target instanceof L2PcInstance) {
            ...
            if (BotsPreventionManager.getInstance()._monsterscounter.containsKey(this.getObjectId())) {
                BotsPreventionManager.getInstance()._monsterscounter.remove(this.getObjectId());
            }
        ...
        if ((TvT.is_started() && _inEventTvT && player_target._inEventTvT) || (DM.is_started() && _inEventDM && player_target._inEventDM) || (BattleRoyale.is_started() && _inEventBR && player_target._inEventBR) || (CTF.is_started() && _inEventCTF && player_target._inEventCTF) || (VIP._started && _inEventVIP && player_target._inEventVIP))
            return;
    ...
    public int getRunSpeed() {
        ...
        int[] classBonus = new int[]{
                ...
                0x5d, // Adventurer
                0x64, // Eva Templar
                0x6a, // Shillien Templar
                0x6b, // Spectral Dancer
                0x6c, // Ghost Hunter
                0x71 // Titan
        };

        if (ArrayUtils.contains(classBonus, getClassId().getId())) {
            bonus = 20;
        }
        return getStat().getRunSpeed() + bonus;
    }
    ...
    private boolean createDb() {
        ...
        try {
             ...
             * banchat_time
             * ,
             */ + ",name_color,title_color,aio,aio_end,arena_wins,arena_loses,arena_points,daypvp,pvpmaster,last_hop_vote,last_top_vote,last_net_vote,arenapt_wins,arenapt_loses,arenapt_points) " + "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            statement.setString(1, _accountName);
            statement.setInt(2, getObjectId());
            ...
            statement.setInt(64, 0);
            statement.setInt(65, 0);
            statement.setInt(66, 0);
            statement.setLong(67, System.currentTimeMillis() - 43200000);
            statement.setLong(68, System.currentTimeMillis() - 43200000);
            statement.setLong(69, System.currentTimeMillis() - 43200000);
            statement.setInt(70, 0);
            statement.setInt(71, 0);
    ...
    private synchronized void storeCharBase() {
        ...
        try {
            ...
            if (_isInOfflineMode || isOnline() == 1) { // in offline mode or online
                statement.setInt(37, 1);
            } else
                statement.setInt(37, isOnline());
            ...
            statement.setInt(63, getDayPvp());
            statement.setInt(64, isPvpMaster() ? 1 : 0);
    ...
    /**
     * base all ski
     * Retrieve from the datalls of this L2PcInstance and add them to _skills.<BR>
     * <BR>
     */
    ...
    public boolean isAutoAttackable(final L2Character attacker) {
        ...
        if (attacker instanceof L2PlayableInstance && (isInFunEvent() || L2ArenaManagerInstance.inFightOrWaiting.contains(this))) {
            ...
            if (player != null) {
                ...
                if (player.isInFunEvent()) {
                    ...
                    // checks for events
                    if ((_inEventTvT && player._inEventTvT && TvT.is_started() && !_teamNameTvT.equals(player._teamNameTvT)) || (_inEventCTF && player._inEventCTF && CTF.is_started() && !_teamNameCTF.equals(player._teamNameCTF)) || (_inEventDM && player._inEventDM && DM.is_started()) || (_inEventVIP && player._inEventVIP && VIP._started) || (_inEventBR && player._inEventBR && BattleRoyale.is_started())) {
                        return true;
                    }
                    return false;
                } else if (L2ArenaManagerInstance.inFightOrWaiting.contains(this) &&
                        L2ArenaManagerInstance.inFightOrWaiting.contains((L2PcInstance) attacker)
                        && (!this.isInParty() || !((L2PcInstance) attacker).isInParty() || (!this.isInParty() && getParty().getPartyMembers().contains(attacker)))) {
                    return true;
                }
                return false;
            }
    ...
    public boolean checkPvpSkill(L2Object target, final L2Skill skill, final boolean srcIsSummon) {
        ...
        if (target instanceof L2PcInstance) {
            ...
            if (skill.isOffensive() && (_inEventTvT && ((L2PcInstance) target)._inEventTvT && TvT.is_started() && !_teamNameTvT.equals(((L2PcInstance) target)._teamNameTvT)) || (_inEventCTF && ((L2PcInstance) target)._inEventCTF && CTF.is_started() && !_teamNameCTF.equals(((L2PcInstance) target)._teamNameCTF)) || (_inEventDM && ((L2PcInstance) target)._inEventDM && DM.is_started()) || (_inEventBR && ((L2PcInstance) target)._inEventBR && BattleRoyale.is_started()) || (_inEventVIP && ((L2PcInstance) target)._inEventVIP && VIP._started)) {
                return true;
            } else if (L2ArenaManagerInstance.inFightOrWaiting.contains(this) && L2ArenaManagerInstance.inFightOrWaiting.contains(((L2PcInstance) target)) && (this.getParty() == null || !this.getParty().getPartyMembers().contains(((L2PcInstance) target)))) {
                if (!(skill.getId() == 1305) && !(skill.getId() == 1416)) {
                    return true;
                } else {
                    return false;
                }
            } else if (isInFunEvent() && !_inEventBR && skill.isOffensive() && !BattleRoyale.is_joining()) // same team return false
            {
                return false;
            }
        }
    ...
    public void updateNameTitleColor() {
        if (isMarried() && Config.L2JMOD_ALLOW_WEDDING_COLOR) {
            if (marriedType() == 1) {
    ...
    public synchronized boolean addSubClass(final int classId, final int classIndex) {
        ...
        if (classId == 115 || classId == 118 || classId == 51 || classId == 50 || classId == 49 || classId == 57 || classId == 56 || classId == 53) {
            return false;
        }
    ...
    public final void sendDamageMessage(final L2Character target, final int damage, final boolean mcrit, final boolean pcrit, final boolean miss) {
        ...
        if (target instanceof L2PcInstance) {
            if (((L2PcInstance) target).isTeleportProtected()) {
                this.sendMessage("Target is currently under teleport protection.");
                return;
            }
        }
        ...
        switch (state) {
            ...
            case JAIL: // Jail Player
            {
                ...
                if (_inEventCTF) {
                    ...
                    BattleRoyale.onDisconnect(this);
                } else if (_inEventTvT) {
