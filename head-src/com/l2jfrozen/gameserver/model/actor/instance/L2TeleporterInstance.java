...
public final class L2TeleporterInstance extends L2FolkInstance
{
	...
	private void doTeleport(final L2PcInstance player, final int val)
	{
		...
		if (list != null)
		{
			...
			}else if(list.getTeleId() == 77777){
				player.teleToLocation(PvpZonesData.getInstance().pvpZones.get(PvpZoneChangeManager.getInstance().currentZone).getTeleportLoc(), true);
			}
