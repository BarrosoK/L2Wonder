			...
			switch (val)
			{
				...
				case 4:
					if(Olympiad.getInstance().isRegistered(player)){
						player.sendMessage("You are already registered to Olympiad");
						return;
					}
					...
					else if((player.isInParty() && L2ArenaManagerInstance.participantsLobby.contains(player.getParty()))||L2ArenaManagerInstance.participants.contains(player)){
						player.sendMessage("You are already registered in street fight arena.");
						return;
					}
