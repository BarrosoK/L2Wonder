...
public abstract class L2Character extends L2Object implements ISkillsHolder {
     ...
     */
    public static final int ZONE_MULTIFUNCTION = 40000;

    /**
     * The Constant ZONE_PARTYPVP.
     */
    public static final int ZONE_PARTYPVP = 40001;

    public static final int ZONE_PARTYWAITINGROOM = 40002;

    public static final int ZONE_CUSTOMARENA = 40003;
    /**
    ...
    private boolean doAttackHitByBow(final Attack attack, final L2Character target, final int sAtk, final int reuse) {
        ...
        _disableBowSkills = ((sAtk + reuse) / GameTimeController.MILLIS_IN_TICK)*40/100 ;
    ...
    public void doCast(final L2Skill skill) {
        ...
        if (activeChar instanceof L2PcInstance && !skill.isPotion()) {
            ...
            if ((rhand != null && rhand.getItemType() == L2WeaponType.BOW)) {
                if (isAttackingNow()|| (_disableBowAttackEndTime-_disableBowSkills) > GameTimeController.getGameTicks())
                    return;
            }
        }
        ...
        if (!Config.PLAYERS_CAN_HEAL_RB && activeChar instanceof L2PcInstance && !((L2PcInstance) activeChar).isGM() && (target instanceof L2RaidBossInstance || target instanceof L2GrandBossInstance) && (skill.getSkillType() == SkillType.HEAL || skill.getSkillType() == SkillType.HEAL_PERCENT || skill.getSkillType() == SkillType.BUFF)) {
            this.sendPacket(ActionFailed.STATIC_PACKET);
            return;
        }

        /*
        if (activeChar instanceof L2PcInstance && activeChar.getPet() != null && !(((L2PcInstance) activeChar).isInOlympiadMode())) {
            final L2Summon summon = activeChar.getPet();
            if (!((L2PcInstance) activeChar).isGM() && target instanceof L2PetInstance && target != summon && (skill.getSkillType() == SkillType.BUFF || skill.getSkillType() == L2Skill.SkillType.FORCE_BUFF)) {
                return;
            }
        } else if (activeChar instanceof L2PcInstance && activeChar.getPet() == null && !((L2PcInstance)activeChar).isInOlympiadMode()) {
            if (!((L2PcInstance)activeChar).isGM() && target instanceof L2PetInstance && (skill.getSkillType() == SkillType.BUFF|| skill.getSkillType() == L2Skill.SkillType.FORCE_BUFF)) {
                return;
            }
        }
*/
    ...
    public boolean doDie(final L2Character killer) {
        ...
        // Same thing if the Character isn't a Noblesse Blessed L2PlayableInstance
            ...
            if (this instanceof L2PcInstance) {
                ...
                if (player._inEventDM && DM.is_started()) {
                    if (Config.DM_REMOVE_BUFFS_ON_DIE)
                        stopAllEffects();
                }else if (player._inEventBR && BattleRoyale.is_started()) {
                    if (Config.BR_REMOVE_BUFFS_ON_DIE)
                        stopAllEffects();
            ...
            {
                /*
                if(this instanceof L2MonsterInstance && killer instanceof L2PcInstance){

                    if(this.isChampion()){

                        ((L2PcInstance) killer).addItem("Quest", 4326, 1, killer, true);
                        PlaySound ps = new PlaySound(1, "ItemSound.quest_itemget", 0, 0, 0, 0, 0);
    ...
    class EnableSkill implements Runnable {
         ...
         * @param skill the skill
         */
        private EnableSkill(final L2Skill skill) {
            _skillId = skill;
        }
        ...
        if (src != null && dst != null) {
            ...
            if (dst.isInFunEvent() && src.isInFunEvent()) {
                ...
                if (src.isInStartedTVTEvent() && dst.isInStartedTVTEvent())
                    ...
                    return false;
                else if (src.isInStartedBREvent() && dst.isInStartedBREvent())
                    return false;
                else if (src.isInStartedCTFEvent() && dst.isInStartedCTFEvent())
                    return false;
                else if (src.isInStartedVIPEvent() && dst.isInStartedVIPEvent())
                    return false;
                else if (L2ArenaManagerInstance.inFightOrWaiting.contains(src)&& L2ArenaManagerInstance.inFightOrWaiting.contains(target))
                    return false;
                // else
    ...
    public L2Skill removeSkill(final int skillId, final boolean cancelEffect) {
        ...
        if (oldSkill != null) {
            ...
            if (cancelEffect || oldSkill.isToggle()) {
                ...
                if (e == null && (oldSkill.getId() < 3080 || oldSkill.getId() > 3259)) {
                    removeStatsOwner(oldSkill);
    ...
    public void callSkill(final L2Skill skill, final L2Object[] targets) {
        try {
            ...
            for (final L2Object target : targets) {
                if (target instanceof L2Character) {
                    ...
                    if (Config.ALLOW_RAID_BOSS_PETRIFIED && (this instanceof L2PcInstance || this instanceof L2Summon)) // Check if option is True Or False.
                    {
                        ...
                        // check on BossZone raid lvl
                        if (!(player.getTarget() instanceof L2PlayableInstance) && !(player.getTarget() instanceof L2SummonInstance)) { // this must work just on mobs/raids
            ...
            if ((skill.getId() != 2037 && skill.getId() != 2005 && (skill.getId() != 2166 && skill.getLevel() == 2)) && skill.isPotion() && this instanceof L2PlayableInstance) {
    ...
    public int getMAtkCustom(final L2Character target, final L2Skill skill) {
        return getStat().getMAtkCustom(target, skill);
    }
