...
public class L2MultiFunctionZone extends L2ZoneType{
    ...
    public static ArrayList<L2PcInstance> getList() {
        return list;
    }
    ...
    protected void onEnter(L2Character character) {
        ...
        if (character instanceof L2PcInstance) {
            ...
            activeChar.isInsideMultifunctionZone = true;
            if(!list.contains(activeChar)) {
                list.add(activeChar);
            }
            ...
            */
            activeChar.sendPacket(new ExShowScreenMessage("To exit the PvP Zone press .exit",3000));
            clear(activeChar);
            if (give_noblesse)
    ...
    protected void onExit(L2Character character) {
        if (character instanceof L2PcInstance) {
            L2PcInstance activeChar = ((L2PcInstance) character);
            if(list.contains(activeChar)) {
                list.remove(activeChar);
            }
            activeChar.isInsideMultifunctionZone = false;
            activeChar.enableAllSkills();
            activeChar.setPvpFlagLasts(System.currentTimeMillis() + Config.PVP_NORMAL_TIME);
    ...
    public static void givereward(L2PcInstance player) {
        if (player.isInsideMultifunctionZone) {
