...
public class L2ArenaZone extends L2ZoneType {
    ...
    protected void onEnter(final L2Character character) {
        ...
        if (character instanceof L2PcInstance) {
            if (this.getId() == 11012) {
                ((L2PcInstance) character).isInsideColiseumZone = false;
    ...
    public void removeAllOthers(Vector<L2PcInstance> tvtList){
        for(L2PcInstance p : L2World.getInstance().getAllPlayers()){
            if(p.isInsideColiseumZone && !tvtList.contains(p)){
                p.teleToLocation(147531,46726,-3400,true);
            }
        }
    }
