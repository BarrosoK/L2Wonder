...
public class Orfen extends Quest implements Runnable
{
	...
	private static String msg1 = "I shall show no mercy! No one comes here and walks out alive!";
	private static String msg2 = "I shall show you my true POWER!";
	private static String msg3 = "No...This...can't...be..!";
	...
	public String onAttack(final L2NpcInstance npc, final L2PcInstance attacker, final int damage, final boolean isPet)
	{
		...
		if (npcId == ORFEN)
		{
			...
			if (!attacked)
			{
				...
				npc.broadcastPacket(new CreatureSay(objId, 0,"Orfen" , msg1));
				attacked = true;
			}

			if(!halfHp && npc.getCurrentHp()<npc.getMaxHp()/2){
				final int objId = npc.getObjectId();
				npc.broadcastPacket(new CreatureSay(objId, 0,"Orfen" , msg2));
				halfHp = true;
			}
	...
	public String onKill(final L2NpcInstance npc, final L2PcInstance killer, final boolean isPet)
	{
		if (npc.getNpcId() == ORFEN)
		{
			...
			final int objId = npc.getObjectId();
			npc.broadcastPacket(new CreatureSay(objId, 0, "Orfen", msg3));
			attacked = false;
			halfHp = false;
			...
			if (killer.getClan() != null) {
				Announcements.getInstance().announceToAll("Orfen was killed by " + killer.getName() + " of Clan " + killer.getClan().getName() + ".");
			} else {
				Announcements.getInstance().announceToAll("Orfen was killed by " + killer.getName() + ".");
			}
		}
