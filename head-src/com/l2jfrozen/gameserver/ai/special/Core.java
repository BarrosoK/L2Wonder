...
public class Core extends Quest implements Runnable
{
	...
	private static String msg1 = "Who disturbs my program...";
	private static String msg2 = "You are damaging my circuit board. Stop...";
	private static String msg3 = "System error...";
	...
	public String onAttack(final L2NpcInstance npc, final L2PcInstance attacker, final int damage, final boolean isPet)
	{
		if (npc.getNpcId() == CORE)
		{
			...
			if (!attacked)
			{
				...
				npc.broadcastPacket(new CreatureSay(objId, 0,"Core" , msg1));
				attacked = true;
			}

			if(!halfHp && npc.getCurrentHp()<npc.getMaxHp()/2){
				final int objId = npc.getObjectId();
				npc.broadcastPacket(new CreatureSay(objId, 0,"Core" , msg2));
				halfHp = true;
			}
	...
	public String onKill(final L2NpcInstance npc, final L2PcInstance killer, final boolean isPet)
	{
		...
		if (npcId == CORE)
		{
			...
			final int objId = npc.getObjectId();
			npc.broadcastPacket(new CreatureSay(objId, 0, "Core", msg3));
			attacked = false;
			halfHp = false;
			...
			if (killer.getClan() != null) {
				Announcements.getInstance().announceToAll("Core was killed by " + killer.getName() + " of Clan " + killer.getClan().getName() + ".");
			} else {
				Announcements.getInstance().announceToAll("Core was killed by " + killer.getName() + ".");
			}
		}
		else
		{
