...
public class QueenAnt extends Quest implements Runnable
{
	...
	private static String msg1 = "How dare you defy me! I am the Queen of this layer!";
	private static String msg2 = "Such power....";
	private static String msg3 = "How is this possible!? I am, defeated.";
	...
	public String onAttack(final L2NpcInstance npc, final L2PcInstance attacker, final int damage, final boolean isPet)
	{
		...
		}else if (npc.getNpcId() == QUEEN)
		{
			...
			if (!attacked)
			{
				...
				npc.broadcastPacket(new CreatureSay(objId, 0,"Queen Ant" , msg1));
				attacked = true;
			}

			if(!halfHp && npc.getCurrentHp()<npc.getMaxHp()/2){
				final int objId = npc.getObjectId();
				npc.broadcastPacket(new CreatureSay(objId, 0,"Queen Ant" , msg2));
				halfHp = true;
			}
	...
	public String onKill(final L2NpcInstance npc, final L2PcInstance killer, final boolean isPet)
	{
		...
		if (npcId == QUEEN)
		{
			...
			final int objId = npc.getObjectId();
			npc.broadcastPacket(new CreatureSay(objId, 0, "Queen Ant", msg3));
			attacked = false;
			halfHp = false;
			...
			if (killer.getClan() != null) {
				Announcements.getInstance().announceToAll("Queen Ant was killed by " + killer.getName() + " of Clan " + killer.getClan().getName() + ".");
			} else {
				Announcements.getInstance().announceToAll("Queen Ant was killed by " + killer.getName() + ".");
			}
