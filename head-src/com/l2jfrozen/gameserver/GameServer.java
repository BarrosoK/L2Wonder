...
public class GameServer {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        RegisterSiege rg = new RegisterSiege(99998, "RegisterSiege", "custom");
        Services sec = new Services(99999, "Services", "custom");
        ...
        LOGGER.info("Trivia System activated.");

        ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(PvpZoneChangeManager.getInstance(), 0, 1000 * 60 * 60);
        LOGGER.info("PvP Change System activated.");
        LOGGER.info("First Zone: " + PvpZonesData.getInstance().pvpZones.get(PvpZoneChangeManager.getCurrentZone()).getName() + ".");

        ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(VoteSpam.getInstance(), 5 * 60 * 1000, 45 * 60 * 1000);

        VoteRewardData.getInstance().load();

        if (Config.ALLOW_FAKEPLAYERS_SYSTEM) {
            FakeOnline.restoreFakePlayers();
            //ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(FakeOnlineHeadingValidator.getInstance(),1000,1000);
        }
